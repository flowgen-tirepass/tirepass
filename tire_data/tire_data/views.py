...
def customer_selection(request):
    ...
    if search_query:
        customers = customers.filter(
            ...
            Q(enno__icontains=search_query) |
    ...
    # 등급 필터 (추후 구현)
    if grade_filter:
        # 등급 필드가 추가되면 필터링 로직 추가
        pass

    # 페이지네이션
    paginator = Paginator(customers, 100)  # 페이지당 100개
    ...
    return render(request, 'tire_data/customer_selection.html', context)
