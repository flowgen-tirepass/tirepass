...
# PythonAnywhere 원격 DB 사용하도록 환경변수 설정
os.environ['PYTHONANYWHERE_SITE'] = 'true'
os.environ['DB_PASSWORD'] = '#flowgen9569yjm*'
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'itire.settings')

print("=" * 80)
print("원격 데이터베이스(PythonAnywhere)에 연결 중...")
print("=" * 80)
...
try:
    ...
    print("✓ 원격 데이터베이스 연결 성공!\n")
except Exception as e:
    print(f"✗ 데이터베이스 연결 실패: {e}")
    print("\n가능한 원인:")
    print("  1. 인터넷 연결이 끊어져 있음")
    print("  2. PythonAnywhere 서버가 다운됨")
    print("  3. 방화벽이 MySQL 포트(3306)를 차단함")
    sys.exit(1)
...
if orders.exists():
    ...
    for order in orders:
        ...
        try:
            ...
            print(f"결제 정보: 없음 (결제 미완료)")
        except Payment.MultipleObjectsReturned:
            payments_count = Payment.objects.filter(order=order).count()
            print(f"결제 정보: {payments_count}개 존재 (비정상)")
...
if problem_orders.exists():
    ...
    for order in problem_orders:
        ...
        try:
            payment = Payment.objects.get(order=order)
            print(f"Payment 존재: 예")
            print(f"  ├─ 결제상태: {payment.payment_status}")
            print(f"  ├─ Payment Key: {payment.payment_key or '없음'}")
            print(f"  └─ Memo: {payment.memo or '없음'}")
        except Payment.DoesNotExist:
            print(f"Payment 존재: 아니오 ⚠️")
